...
const GroupChatCreateModal = ({ setAlert, onClose, user }) => {
  ...
  async function postNewChat() {
    try {
      // // Create new chat
      let data = await createNewChat(user.user_id, chatName);
  ...
  const debouncedFetchSearchMatchingUsers = debounce(
    fetchSearchMatchingUsers,
    300
  );
      ...
      >
        ...
        <div className="mb-4 relative">
          ...
          {matchingUsers.length > 0 && (
            <div
              key={matchingUsers.user_id}
              className="absolute z-50 bg-white shadow-lg border-b-2 border-l-2 border-r-2 border-gray-400 w-full max-h-48 overflow-y-auto mt-1"
            >
              {matchingUsers.slice(0, 6).map((user) => (
                <div
                  ...
                  onClick={() =>
                    console.log(
                      `Selected user: ${user.first_name} ${user.last_name}`
                    )
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {pickedUsers.map((user) => (
              <div
                key={user.user_id}
                className="bg-green-500 text-white rounded-full py-1 px-3 flex items-center"
              >
